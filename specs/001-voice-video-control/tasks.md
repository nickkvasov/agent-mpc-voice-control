...
- [X] T135 [P] [US1] Playwright: "go back a bit" acknowledged within one second and applied when the assistant acts; pause pressed before the assistant acts applies and the seek is refused `overtaken_by_newer_command` (FR-038, SC-001); a spent allowance shows the assistant unavailable and, on an idle page with the clock advanced past `resetsAt`, becomes available again and the next command is admitted (Phase 12 Gate C: the reset must not wait for a re-render), in `tests/e2e/assistant-playback.spec.ts`
- [X] T136 [P] [US2] Playwright: find, then "only the short ones" spending no quota, then "play the third one"; with the results view closed, the assistant is told the view is not open rather than acting on a stale listing (FR-016, FR-035, SC-012), in `tests/e2e/assistant-discovery.spec.ts`. Needed, and added: a Hide results toggle (the results view could not be closed); a failed call to a tool the page does not declare comes back `view_not_open` naming the view; and the loop keeps every tool name shown to the model during the turn, so a tool that left the listing mid-turn reaches the page by its own name
- [X] T137 [P] [US3] Playwright: every assistant tool call appears once in the activity record under its command; an assistant entry can be undone; "what did you just do?" matches the record; an assistant call refused before its handler (bad arguments) is recorded WITH its arguments and `commandId` (FR-029, FR-030, FR-033), in `tests/e2e/assistant-activity.spec.ts`
- [X] T138 [P] [US4] Playwright: an assistant removal names its target and waits; "maybe" refuses; a bulk change over five states the count (FR-026, FR-027, FR-028), in `tests/e2e/assistant-curation.spec.ts`. Needed, and added: `curation.getCollections` — without it the assistant could find no collection it had not just created (contract updated). Gate B then found confirmations and record entries naming videos by id, and an unclear answer reported as "…? Confirm to go ahead.", which the live model read as a question still open; both fixed
