...
import { HARNESS, callsOf, captions, config, stamp, turn } from './app.mjs';

const { record, sleep } = await import(HARNESS);

await record({ ...config, name: `voice-video-tour-${stamp()}` }, async ({ page, say: sayOne, card, ask, turnEnds, humanClick, humanType, point, focus, pan, text }) => {
  ...
  // would type it.
  // Every caption goes through this: one that does not fit the band plays as consecutive screens.
  const say = captions(page, sayOne);
  ...
  await say('agent', `It called ${findCalls.map((c) => c.split(' ')[0]).join(', then ')}.`, 3000);
  ...
  await pan('top', 1400);
  // Every beat opens with its own caption: without one, the previous beat's claim stayed on screen
  // over this action (take of 2026-09-14 11:46).
  await say('human', '“Play the first one.”', 0);
  ...
  await say('agent', playing ? `Playing “${nowPlaying}” — the real YouTube embed, confirmed by the player.` : `The player reports ${await stateText()}.`, 4000);
  ...
  await pan('top', 1200);
  await say('human', '“Play” — resumed the same way, on the page.', 0);
  await ask('play');
  ...
  await say('agent', overtaken !== undefined
    ...
    : `This time the assistant acted first: ${backCalls.join(' · ')}`, 4600);

  // ── 5. Every assistant action is recorded, and can be undone ─────────────
  // Reported from the page, never assumed. In the take of 2026-09-14 11:41 the model answered that the
  // queue and results were not on screen — all 34 tools were declared — and the old caption still said
  // "Queued". A refusal like that is shown as it happened, and the request is made once more, plainly.
  const queueCount = () => page.locator('[data-testid="queue-item"]').count();
  await say('human', '“Queue the second one.”', 0);
  await assistant('queue the second one');

  await focus('[data-testid="queue"]', 1600);
  if ((await queueCount()) === 0) {
    const reply = (await turn(page, 'queue the second one').locator('[data-testid="turn-message"]').allInnerTexts()).at(-1) ?? '';
    await say('agent', `Nothing was queued. The assistant replied: “${reply.trim()}”`, 4200);
    await say('human', 'Asked again, naming it plainly.', 1800);
    await pan('top', 1200);
    await assistant('add the second search result to the queue');
    await focus('[data-testid="queue"]', 1600);
    if ((await queueCount()) === 0) throw new Error('the assistant queued nothing on the second request either');
  }
  await say('agent', `Queued — the queue now holds ${await queueCount()}.`, 2600);
  ...
  await say('human', `Undo “${undoTarget}”.`, 1800);
  ...
  await say('note', lastQuestion === null ? 'The page did not ask anything.' : `The page asked “${lastQuestion}” — and the answer typed was “maybe”.`, 4200);
  await point('[data-testid="assistant-turn"] ul', 1800);
  ...
  await say('agent', refusedRemoval && left === 1
    ...
    : `Outcome: ${removeCalls.join(' · ')} — ${left} video(s) left.`, 4400);
